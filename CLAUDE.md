...
- Prefer caution over speed for non-trivial changes.
- State assumptions when they affect implementation.
- Ask when requirements are ambiguous or when multiple plausible interpretations would lead to different code.
...
- Match existing conventions even if a different style seems preferable.
- Before editing, inspect relevant exports, callers, shared utilities, and nearby patterns.

## Verification

- Define what success means before implementing.
- Verify the change with the most relevant available tests, type checks, lint checks, or manual checks.
- If verification cannot be run, say so explicitly.
- Do not claim tests pass if they were skipped, unavailable, or only partially run.

## Tests

- Add or update tests when behavior changes.
- Tests should encode the reason the behavior matters, not just assert implementation details.
- Avoid brittle tests that only mirror the current code.
...
- Surface conflicts between existing patterns instead of blending them.
- If confused, stop and name what is unclear.
- Never silently skip requested work.