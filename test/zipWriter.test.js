...
test("createZip writes local headers, a central directory, and the end record", () => {
  const zip = createZip([
  ...
  assert.equal(readUint16(zip, end + 10), 2);
...
test("a stored entry round-trips byte for byte", () => {
  const payload = '{"levels":["1F","2F"]}';
  const zip = createZip([{ path: "venue.json", data: payload }]);

  const nameLength = readUint16(zip, 26);
  const extraLength = readUint16(zip, 28);
  const dataStart = 30 + nameLength + extraLength;
  const compressedSize = readUint32(zip, 18);
...
test("createZip keeps UTF-8 paths and flags them", () => {
  const zip = createZip([{ path: "layers/縦動線.geojson", data: "{}" }]);
  const nameLength = readUint16(zip, 26);
  assert.equal(
    new TextDecoder().decode(zip.subarray(30, 30 + nameLength)),
    "layers/縦動線.geojson",
...
test("createZip rejects data it cannot store", () => {
  assert.throws(() => createZip([{ path: "bad", data: { nope: true } }]), /must be a string/);
});
